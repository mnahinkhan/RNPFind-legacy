# A file dedicated to useful functions related to position weight matrices that describe the binding profiles of RBPs,
# such as scanning an RNA sequence to detect where the RBP might bind on it.
# Possibly the file I had the most pleasure writing, despite its simplicity.

# WHat is a PWM? A PWM is a position weight matrix that summarizes a collection of strings. In the context of biology,
# an example could be a collection of sequences that an RBP was found to bind to protein. Say, for example, that
# an RBP called HNRNPK was found to bind to 100 RNA sequences, each of length 5 bases. We could then summarize, for each
# of those 5 bases, the frequency of occurrence of a nucleotide base in that position. An example binding profile could
# be:
# Base / Position
#   1   2   3   4   5
# A 50  100 100 0   0
...
# Notice how each column adds up to 100, since we had a collection of 100. We can conclude that this RBP binds to a
# 5-letter sequence that looks like (A/G)AA(G/C/T)G, with the distribution shown above (e.g. first letter is equally
# likely to be A or G, and so on). Technically, what is shown above is a position frequency matrix. It is common to
# normalize the above so each column adds up to 1, to generate a position probability matrix. For some purposes,
# one might take the log values of the above too. I think all these things can be called PWMs. Thus in this project,
# I have referred to any form of position frequency matrix (normalized or not) as a PWM. We do not deal with logs here!

# Clearly, pwms are useful for making predictions about where an RBP might bind to an RNA sequence. I did not do
# extensive research on statistics here to make such predictions; instead I used a simplest model described on a
# Wikipedia page, where one assumes an equiprobable base distribution for prior belief and independence between motif
# base positions in terms of nucleotide preference.

# Specifically, we decide if an rbp binds to a specific location on an RNA sequence by associating a "score" with the
# rbp binding profile on a specific location, calculated by multiplying all the position weights that show up on the
# RNA molecule. The RBP is considered to bind if the score calculated is more than 80% of the highest possible score
# attainable (cutoff percentage can be adjusted in config.py).

# We could have chosen a number of ways to represent PWM's. Somewhat arbitrarily, I went with a pwm variable being
# represented by a dictionary that maps the letters of the nucleotide bases ("A", "C", etc.) to a list of length equal
# to the length of the motif. The list has numbers that represent frequency (not assumed to be probabilities) of the
# base at that position. For example, pwm["T"][6] is the frequency of the base T on the 7th nucleotide of the motif
# represented by "pwm".
...
def get_human_seq(chr_no, chr_start, chr_end):
    """Returns the human genomic sequence of the specified chromosome from the start to the end coordinate."""
    chr_files_dir = "../Raw Data/" + genome_version + " Genome/"
    chr_file = chr_files_dir + "chr" + str(chr_no) + ".fa"
    with open(chr_file) as handle:
        # Need to skip the first line, and then a bunch more to reach the line that has the chr_start numbered base.
        # Note that the .fa file comes in lines of 50 nucleotides with "\n" at the end. Admittedly, a part of the
        # below formula was trial and error until it was empirically correct (maybe the -1 part).
        handle.seek(4 + len(str(chr_no)) + 1 + 51 * (chr_start // 50) + chr_start % 50 - 1)
        # Once at the required position, keep adding lines of bases as long as too short. Then adjust in case we
        # overshot the required length.
        gene = ""
...
def pwm_scan(gene, pwm):
    """Scans gene, a string of nucleotide bases, for positions where pwm could bind. The approach used here is to
...
def motif_to_pwm(motif, letter_strength=4):
    """Converts a motif (a string like "AGCNNNYWS") to a corresponding pwm. In making the pwm, letter_strength (
...
if __name__ == '__main__':
    ...
    print(pwm_scan_naive_brute_force("CCCCCCTTTTCGCGCGCGCTTTTCGCGCGCGCGCGTTTTTTT", motif_to_pwm("TTTT")))
